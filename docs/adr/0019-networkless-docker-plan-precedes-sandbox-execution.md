...
workspace. Workspace paths containing NUL or Docker `--mount`'s comma delimiter
are rejected before command construction, while spaces and platform-native path
separators remain valid. Materialization does not execute Docker or create eligible repair-rate

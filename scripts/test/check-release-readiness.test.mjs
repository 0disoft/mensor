...
test("release check accepts the pnpm argument separator", () => {
  const result = spawnSync(
    process.execPath,
    [script, "--", "--version", "0.7.0", "--tag", "latest"],
    {
      cwd: repositoryRoot,
      encoding: "utf8",
    },
...
Version \`0.7.0\` is the current public preview.

## Registry Installation

\`pnpm add --save-dev @0disoft/mensor-cli@0.7.0\`

See docs/releasing/0.7.0.md.
`;

  assert.deepEqual(validateReleaseReadme({ readme, version: "0.7.0" }), []);
});
...
`;
  ...
  assert.deepEqual(validateReleaseReadme({ readme, version: "0.7.0" }), [
    "README.md Status must identify 0.7.0 as the current public preview.",
    'README.md Registry Installation must contain "pnpm add --save-dev @0disoft/mensor-cli@0.7.0".',
    "README.md Registry Installation must link to docs/releasing/0.7.0.md.",
  ]);
});
